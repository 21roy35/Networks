...
The bot accepts messages only from the configured private chat ID. `/status` reports the current flight, complaint, and mailbox totals. `/ticket` accepts a ticket PDF/photo or pasted itinerary, extracts every visible leg and passenger, asks for any required missing fields, and shows a confirmation preview before adding durable source evidence to the dashboard. `/complaintref C_1234567 flight SV123 date 2026-08-01 filed 2026-08-02 about …` attaches a case filed outside FlightDeck to exactly one matching flight; ambiguous family matches are rejected rather than guessed. `/flightstatus SV1671` refreshes source-attributed live evidence for an exact flight, and `/recommend SV1671` explains its best complaint next step. `/web` returns a short-lived private sign-in link for opening the dashboard on a phone or other device; after opening it, that browser remains signed in for the configured session period. OTP replies are deleted from the Telegram chat after use when Telegram permits deletion. Evidence photos and imported ticket files are downloaded to the ignored local `telegram_evidence/` directory.
